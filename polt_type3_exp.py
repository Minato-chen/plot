...
def fgbgcolor(r=20, min_distance=80):
    while True:
        ...
        if (
            ...
            and color_distance(rgb_fg, rgb_bg) > min_distance
        ):
            break
...
# 绘制图案函数
def plot_pillow(rgb_fg, rgb_bg, size, n, grid_line_factor=8, output_dir="./output_exp"):
    # 创建一个填充前景色的图像
    ...
    output_dir = os.path.join(output_dir, "{}_{}".format(n, grid_line_factor))
...
# n = 16  # 指的是长或宽为16个小正方形
# grid_line_factor = 2  # 指的是网格线的宽度影响因子，值越大，网格线越细

for i in range(2):
    rgb_fg, rgb_bg = fgbgcolor(r=20, min_distance=80)
    plot_pillow(rgb_fg, rgb_bg, 720, 32, 2)
